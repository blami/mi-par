...
int main(int argc, char **argv) {
	int l;                  // delka zpravy se zadanim
	char *b;                // buffer na zpravu se zadanim

	unsigned int n,k,q;     // charakteristiky zadani
	...
	// broadcastovat velikost zpravy se zadanim, aby ostatni uzly mohly
	// provest alokaci bufferu
	MPI_Bcast(&l, 1, MPI_INT, 0, MPI_COMM_WORLD);

	if(node > 0)
		b = (char *)utils_malloc(l * sizeof(char));

	// broadcastovat zpravu se zadanim a sestavit strukturu t na vsech uzlech
	...
	if(node > 0) {
		...
		assert(t);
		srpdebug("mpi", node, "zadani prijato");
	}
	free(b); b = NULL;
